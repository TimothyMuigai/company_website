...
export async function POST(req: NextRequest) {
	try {
		...
		// Save to MongoDB only when a database is configured for this deployment.
		if (!clientPromise || !MONGODB_DB) {
			return new Response(JSON.stringify({ message: 'Database is not configured' }), { status: 503 });
		}
		try {
